...
public class MySqlPagingInterceptor extends PageInterceptor {
    ...
    protected String getSelectTotalSql(String targetSql) {
        String sql = targetSql;//targetSql.toLowerCase();
...
//        }
        ...
        return "select count(*) from ("+sqlBuilder+")as t";
    }
    ...
    protected String getSelectPagingSql(String targetSql, PageBean bounds) {
    	...
    	if(order_index!=-1){
    		...
    		str=str+") as a "+sql.substring(order_index, sql.length())+" limit "+(bounds.getPage()-1)*bounds.getPageSize()+","+bounds.getPageSize();
    	}else{  
	        str="select * from (";
	        str=str+sql;
	        str=str+") as a	limit "+(bounds.getPage()-1)*bounds.getPageSize()+","+bounds.getPageSize();
    	}
        return str;
    }
